from nsga2_gp_example.population import Population
import random
...
class NSGA2Utils:
    ...
    def __mutate(self, child):
        ...
        for gene in range(num_of_features):
            ...
            if u < 0.5:
                child.features[gene] += delta * (child.features[gene] - child.range[0])
            else:
                child.features[gene] += delta * (child.range[1] - child.features[gene])
            if child.features[gene] < child.range[0]:
                child.features[gene] = child.range[0]
            elif child.features[gene] > child.range[1]:
                child.features[gene] = child.range[1]

    def __get_delta(self):
        u = random.random()
        if u < 0.5:
            return u, (2 * u) ** (1 / (self.mutation_param + 1)) - 1 # self.mutation_param = 5
